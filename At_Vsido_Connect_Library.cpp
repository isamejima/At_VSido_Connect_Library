...
//  角度情報の統合
short At_Vsido_Connect_Library::uniAngle(
...
//  角度情報の分割
void At_Vsido_Connect_Library::divAngle(
    short data, unsigned char *upper, unsigned char *lower) {
	UNIWORD uniWord;  // データ変換用共用体
...
bool At_Vsido_Connect_Library::read1byte(unsigned char ch) {
	...
	if (ch != 0xFF && pc_cnt == 0) return false;
	...
	if (ch == 0xFF) {  //スタートフラグ
		...
		if (!isValidOP(ch)) {  //知らないオペランドの場合は受信をリセット
			resetRead1byte();
			return false;
		}
		pc_op = ch;
...
bool At_Vsido_Connect_Library::unpackObjectPacket() {
	...
	for (int i = 0; i < servo_num; i++) {
		...
		int servo_angle = uniAngle(
		    pc_rstr[read_offset + i * 3 + 1], pc_rstr[read_offset + i * 3 + 2]);
		//例外値でなければ更新
...
unsigned char At_Vsido_Connect_Library::calcSum(
    const unsigned char packet[], int packet_ln) {
	unsigned char sum = 0;
...
void At_Vsido_Connect_Library::genVSidoPacket(unsigned char op,
    const unsigned char data[], int data_ln, unsigned char *packet,
    int *packet_len) {
	int cnt     = 0;
	*packet_len = 0;
...
void At_Vsido_Connect_Library::genVSidoCmd(
    unsigned char r_op, const unsigned char data[], int data_ln) {
	genVSidoPacket(r_op, data, data_ln, r_str, &r_ln);
}

unsigned char At_Vsido_Connect_Library::getStatusByte(int id) {
	if (!isValidServoID(id)) return MASK_ERROR;
