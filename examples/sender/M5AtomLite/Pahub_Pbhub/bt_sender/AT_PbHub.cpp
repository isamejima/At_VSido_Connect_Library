...
void AT_PbHub::hub_wire_setServoAngle_A(uint8_t reg, uint8_t angle) {

  if (angle < 0 || angle > 180)return;

  _pWire->beginTransmission(_iic_addr);
  _pWire->write(reg | 0x0C);
  _pWire->write(angle & 0xff);
  _pWire->endTransmission();
}

void AT_PbHub::hub_wire_setServoAngle_B(uint8_t reg, uint8_t angle) {

  if (angle < 0 || angle > 180)return;

  _pWire->beginTransmission(_iic_addr);
  _pWire->write(reg | 0x0D);
  _pWire->write(angle & 0xff);
  _pWire->endTransmission();
}

void AT_PbHub::hub_wire_setServoPulse_A(uint8_t reg, uint16_t pulse) {

  if (pulse < 500 || pulse > 2500)return;

  _pWire->beginTransmission(_iic_addr);
  _pWire->write(reg | 0x0E);
  _pWire->write(pulse & 0xff);
  _pWire->write(pulse >> 8);
  _pWire->endTransmission();
}

void AT_PbHub::hub_wire_setServoPulse_B(uint8_t reg, uint16_t pulse) {

  if (pulse < 500 || pulse > 2500)return;

  _pWire->beginTransmission(_iic_addr);
  _pWire->write(reg | 0x0F);
  _pWire->write(pulse & 0xff);
  _pWire->write(pulse >> 8);
  _pWire->endTransmission();
}
