...
bool At_Vsido_Connect_Sender::genObjectPacket(
...
void At_Vsido_Connect_Sender::setControlTable(
    int id, AT_Vsido_Control_Table table) {
}
AT_Vsido_Control_Table At_Vsido_Connect_Sender::getControlTable(int id) {
	AT_Vsido_Control_Table table = {0};
	if (!isValidServoID(id)) return table;
...
AT_Vsido_Control_Table At_Vsido_Connect_Sender::TableConvertToProtocol(
    AT_Vsido_Control_Table raw_table) {
...
AT_Vsido_Control_Table At_Vsido_Connect_Sender::TableConvertFromProtocol(
    AT_Vsido_Control_Table vsido_table) {
...
bool At_Vsido_Connect_Sender::unpackObjectPacket() {
	...
	for (int i = 0; i < servo_num; i++) {
		...
		int servo_angle = uniAngle(
		    pc_rstr[read_offset + i * 4 + 2], pc_rstr[read_offset + i * 4 + 3]);
		//例外値でなければ更新
...
bool At_Vsido_Connect_Sender::genDataPacket(
    unsigned char *packet, int *packet_ln) {
