...
class At_Vsido_Connect_Library {
 public:
  ...
  //外部利用のみで内部参照しない値
  int servo_angles_ofset
      [VSIDO_MAXSERVO];  //関節角度補正値（今は外部から利用するだけで内部参照していない）
  bool servo_connected
      [VSIDO_MAXSERVO];  //全関節接続状態（今は外部から利用するだけで内部参照していない）

  //通信用
  int MAXPACKETLEN =
      VSIDO_MAXPACKETLEN;  //最大パケット長（これ以上長い場合は受信を中止）
  unsigned char pc_rstr[VSIDO_MAXPACKETBUF] = {};  //受信データ
  int           pc_cnt = 0;  //現在受信している文字数のカウント
  unsigned char pc_op = '_';  //受信中命令のオペランド（角度命令なら'o'等）
  int pc_ln =
      0;  //受信メッセージ中に表記された命令長。４～MAXPACKETLENの間になるはず

  bool read1byte(
      unsigned char
          ch);  //受信文字を追加。1パケット分の命令の受信に成功した時だけtrueを返す

  int pc_timeout =
      0;  //タイムアウト（今は外部から利用するだけで内部参照していない）
 ...
 protected:
  ...
  void genVSidoPacket(unsigned char op, const unsigned char data[], int data_ln,
